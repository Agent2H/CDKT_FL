...
def plot_from_file():
    ...
    if RUNNING_ALG=="CDKT":
        plt.figure(10)
        plt.clf()
        plt.plot(f_data['local_loss'])
        plt.plot(f_data['local_loss'], linestyle="--", label="local loss")
        plt.legend()
        plt.xlabel("Local loss")
        plt.ylim(0.5, 14)
        plt.grid()
        plt.title("Local loss")
        plt.savefig(PLOT_PATH + alg_name + "local_loss.pdf")

        plt.figure(11)
        plt.clf()
        plt.plot(f_data['global_loss'])
        plt.plot(f_data['global_loss'], linestyle="--", label="global loss")
        plt.legend()
        plt.xlabel("global loss")
        plt.ylim(0, 2.0)
        plt.grid()
        plt.title("global loss")
        plt.savefig(PLOT_PATH + alg_name + "global_loss.pdf")
