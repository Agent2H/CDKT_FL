...
class UserMOON(User):
    ...
    def train(self, epochs,global_model,pre_w):
        LOSS = 0
        ...
        for epoch in range(1, epochs + 1):
            ...
            for X,y in self.trainloader:
                ...
                self.optimizer.zero_grad()
                output,rep_output = self.model(X)
                gen_output,gen_rep=gen_model(X)
                pre_output,pre_rep=pre_model(X)
                positive = torch.mean(self.cos(rep_output, gen_rep) / self.temperature)
                negative = torch.mean(self.cos(rep_output, pre_rep) / self.temperature)
                loss_con = -torch.log(torch.exp(positive) / (torch.exp(positive) + torch.exp(negative)))
                # print("loss con", loss_con)
                # proximal_term = 0.0
                # for w, w_t in zip(self.model.parameters(), gen_model.parameters()):
                #     proximal_term += (w - w_t).norm(2)
                loss = self.loss(output, y) + mu*loss_con
                loss.backward()
                self.optimizer.step()
        self.clone_model_paramenter(self.model.parameters(), self.local_model)
        return LOSS
