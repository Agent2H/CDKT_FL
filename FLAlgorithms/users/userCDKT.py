    ...
    def train_distill(self, epochs, global_model,pre_w,glob_iter,alpha1=alpha):
        ...
        for epoch in range(1, epochs + 1):  # local updatec
            ...
            for [local_batch_idx,(X, y)], [batch_idx, (X_public,y_public)] in zip(private_data, public_data):
                ...
                gen_output_public, rep_gen_output_public = gen_model(X_public) #gen model from server

                if Tune_output:
                    y_onehot = F.one_hot(y_public, num_classes=NUMBER_LABEL)
                    gen_output_public = (gen_output_public + y_onehot )/2.0

                # print("gen public label previous",gen_output_public)
                # print("gen public label", gen_output_public)
