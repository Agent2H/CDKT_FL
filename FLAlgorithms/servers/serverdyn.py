...
from FLAlgorithms.users.userdyn import UserDyn
...
class FedDyn(Dem_Server):
    def __init__(self, experiment, device, dataset,algorithm, model, client_model,batch_size, learning_rate, beta, L_k, num_glob_iters, local_epochs, optimizer, num_users, times , cutoff,args):
        ...
        for i in range(self.total_users):
            ...
            user = UserDyn(device, id, train, test, public, model,client_model, batch_size, learning_rate,beta,L_k, local_epochs, optimizer)
        ...
        print("Finished creating FedDyn server.")
    ...
    def update_server_state(self):
        ...
        for client_model in self.uploaded_models:
            for server_param, client_param, delta_param in zip(self.model.parameters(),
                                                               client_model.parameters(), model_delta.parameters()):
                delta_param.data += (client_param - server_param) / N_clients
    ...
    def add_parameters_dyn(self, client_model):
    ...
    def aggregate_parameters_dyn(self):
        assert (len(self.uploaded_models) > 0)
        ...
        for client_model in self.uploaded_models:
            self.add_parameters_dyn(client_model)
    ...
    def receive_models(self):
        ...
        active_clients = random.sample(
            list(self.selected_users), int((1 - self.client_drop_rate) * self.current_num_join_clients))
