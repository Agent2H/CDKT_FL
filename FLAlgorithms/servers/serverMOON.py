...
from FLAlgorithms.users.userMOON import UserMOON
...
class MOON(Dem_Server):
    def __init__(self, experiment, device, dataset,algorithm, model, client_model,batch_size, learning_rate, beta, L_k, num_glob_iters, local_epochs, optimizer, num_users, times , cutoff,args):
        ...
        for i in range(self.total_users):
            ...
            user = UserMOON(device, id, train, test, public, model,client_model, batch_size, learning_rate,beta,L_k, local_epochs, optimizer)
            self.users.append(user)
            self.total_train_samples += user.train_samples

        print("Fraction number of users / total users:",num_users, " / " ,self.total_users)
        print("Finished creating MOON server.")
    ...
    def train(self):
        ...
        prev_w_dict = dict()
        for user in self.users:
            prev_w_dict[user] = copy.deepcopy(self.model)
        for glob_iter in range(self.num_glob_iters):
            ...
            for user in self.selected_users:
                # if(glob_iter==0):
                #     user.train(self.local_epochs,self.model)
                # else:
                    pre_w = prev_w_dict[user]
                    user.train(self.local_epochs,self.model,pre_w)
                    prev_w_dict[user] = copy.deepcopy(user.model)
