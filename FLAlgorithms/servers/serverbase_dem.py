...
class Dem_Server(Server):
    ...
    def gc_test(self,c_model):
        ...
        f1_results=[]
        for c in self.users:
            ct, ns,f1 = c.test_gen(c_model)
            tot_correct.append(ct * 1.0)
            num_samples.append(ns)
            f1_results.append(f1)

        return np.sum(tot_correct), np.sum(num_samples), f1_results
    def evaluating_global_CDKT(self,i):
        ct, ns ,f1= self.gc_test(self.model)  # evaluate global model on all client dataset
        tqdm.write('At round {} global testing accuracy: {}'.format(i, ct / ns))
        tqdm.write('At round {} global F1 accuracy: {}'.format(i, np.mean(f1)))
        self.rs_glob_acc.append(ct / ns)
        self.rs_global_f1.append(np.mean(f1))
    def c_test(self, i, mode="spe"):  # mode spe: specialization, gen: generalization
        ...
        for c in self.selected_users:
            ...
            if (mode == "spe"):
                ...
                if Same_model:
                    ct, ns,f1 = self.gc_test(c.model)  # Test client as testing group approach in gen mode
                else:
                    ct, ns,f1 = self.gc_test(c.client_model)  # Test client as testing group approach in gen mode
            tot_correct.append(ct * 1.0)
            num_samples.append(ns)
            clients_acc.append(ct / ns)
            f1_result.append(f1)
        ...
        ids = [c.id for c in self.users]
        groups = [c.group for c in self.users]
        return ids, groups, num_samples, tot_correct, f1_result
