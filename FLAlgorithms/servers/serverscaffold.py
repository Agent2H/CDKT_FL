...
class Scaffold(Dem_Server):
    def __init__(self, experiment, device, dataset,algorithm, model, client_model,batch_size, learning_rate, beta, L_k, num_glob_iters, local_epochs, optimizer, num_users, times , cutoff,args):
        ...
        for i in range(self.total_users):
            ...
            user = UserScaffold(device, id, train, test, public, model,client_model, batch_size, learning_rate,beta,L_k, local_epochs, optimizer)
    ...
    def send_models(self):
        ...
        for user in self.users:
            ...
            user.set_new_parameters(self.model, self.global_c)
    ...
    def receive_models(self):
        ...
        # active_clients= list(self.selected_users)
    ...
    def aggregate_parameters_scaffold(self):
        ...
        # print("client",self.num_join_clients)
        for cid in self.uploaded_ids:

            dy, dc = self.users[int(cid.split('_')[1])].delta_yc()

            for server_param, client_param in zip(global_model.parameters(), dy):
                server_param.data += client_param.data.clone() / self.num_join_clients * self.server_learning_rate
            for server_param, client_param in zip(global_c, dc):
                server_param.data += client_param.data.clone() / N_clients
